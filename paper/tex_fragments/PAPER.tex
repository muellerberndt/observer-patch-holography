...
\begin{quote}
OPH is an observer-centric framework for fundamental physics. From one calibrated constant (the pixel area), structural axioms (A1-A4 + MAR), and one cosmological input (screen capacity), OPH derives a conditional scaling-limit Einstein branch, compact gauge reconstruction, particle mass hierarchy, and key quantitative outputs. The Standard Model, general relativity, and string theory emerge as effective descriptions of the underlying structure.
\end{quote}
...
\textbf{Theorem 5.1 (Observer-consistency implies the scaling-limit Einstein branch).} Under A1-A4, Assumptions B-G, and the EFT bridge, the cap equilibrium condition implies the rest-frame first-variation Einstein relation, and overlap consistency upgrades it to the semiclassical Einstein equation in the locally Lorentzian scaling regime where the small-diamond modular Hamiltonian is a stress-tensor charge. QED.
...
\item
  Theorem 5.1 (entanglement equilibrium) shows that the condition δS\_gen = 0 implies the rest-frame first-variation Einstein relation, with overlap consistency upgrading it to the semiclassical Einstein equations in the EFT regime.
\item
