...
Assumption D says that gauge is the redundancy of overlap presentations, not a separately postulated microscopic symmetry. Fix a regulated cut \(\Sigma\) with finitely many overlap charts, let \(\varphi_{ij}\) denote the overlap-consistent change of chart from patch \(i\) to patch \(j\) on the shared interface data, and choose a reference boundary chart Hilbertized as \(\tilde{\mathcal H}_\Sigma\). Each \(\varphi_{ij}\) induces a \(^*\)-automorphism of \(\mathcal B(\tilde{\mathcal H}_\Sigma)\), hence is implemented by a unitary \(u_{ij}\in U(\tilde{\mathcal H}_\Sigma)\), unique up to phase. The subgroup generated by these recharting unitaries has compact closure

\[
K_\Sigma:=\overline{\langle u_{ij}\rangle}\subset U(\tilde{\mathcal H}_\Sigma).
\]

This is the boundary redundancy group that feeds the edge-center theorem of Section 2.3 on the ordinary or central-defect branch. Before choosing a reference chart, the same data are more properly described as a compact unitary groupoid of overlap-preserving transitions. Local rechartings \(h_i\in U(\tilde{\mathcal H}_\Sigma)\) act by

\[
u_{ij} \mapsto h_i^{-1}u_{ij}h_j.
\]

\textbf{Lemma 3.2 (trees vs loops).} If the patch adjacency graph is a tree, one can choose local trivializations so that all overlap transports are identity on the boundary data; equivalently, the derived \(K_\Sigma\) trivializes. If loops exist, the ordered product of overlap transports around a cycle \(\gamma=i_1\cdots i_n i_1\),

\[
U(\gamma) := u_{i_1 i_2}u_{i_2 i_3}\cdots u_{i_n i_1},
\]

is invariant up to conjugacy, and up to phase before one passes to a central-extension lift, under local rechartings. Nontrivial loop data are precisely the obstruction to global trivialization.

\textbf{Proof.} On a tree, fix a root chart and transport the trivialization outward; there are no consistency conditions beyond pairwise overlaps, so each \(u_{ij}\) can be absorbed by redefining descendant charts. Around a loop, the ordered product changes by conjugation under \(u_{ij}\mapsto h_i^{-1}u_{ij}h_j\), so its conjugacy class, and projective class before a central lift, is invariant. If that loop product is nontrivial, no global trivialization exists. QED.

This derived compact action is the only group-level input used in the EC theorem. Section 3.3 records the central \v{C}ech-class obstruction to lifting the projective loop data to an honest group action, and Section 3.4 records the genuinely noncentral higher-gauge obstruction when no such ordinary-group reduction exists.
