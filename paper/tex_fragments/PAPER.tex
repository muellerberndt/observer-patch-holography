...
\item
  \textbf{Gauge-sector reconstruction with unique Standard Model closure}: compact gauge symmetry from edge-sector fusion (Theorem 6.1), then unique selection of \(SU(3)\times SU(2)\times U(1)/\mathbb{Z}_6\) with \(N_c=3\), \(N_g=3\) under MAR and admissibility (see GAUGE\_GROUP\_DERIVATION.md).
\item
...
\item
  \textbf{End-to-end particle spectrum derivation from OPH inputs}: from pixel area to couplings and masses, including precision-level agreement for \(W, Z, e, \mu, \tau\) (sub-0.04\%) and percent-level Higgs/top closure (see SPECTRUM\_DERIVATION.md).
\item
  \textbf{String-theory bridge from OPH first principles}: edge-sector MaxEnt weights are exactly 2D Yang-Mills heat kernels, whose large-\(N\) expansion is the Gross-Taylor worldsheet/string expansion (see STRING\_THEORY.md).
\item
  \textbf{Quantitative supplementary closures}: Koide structure, \(\mathbb{Z}_6\) hierarchy suppression, black-hole recovery, modular-anomaly dark-sector phenomenology, baryogenesis scale estimates, and cosmological parameter relations are derived within the same framework (see TECHNICAL\_SUPPLEMENT.md).
\end{enumerate}
...
See GAUGE\_GROUP\_DERIVATION.md for the complete formal statement, admissibility definitions, and proof.
