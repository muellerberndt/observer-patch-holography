...
\bottomrule\noalign{}
\endlastfoot
\textbf{A1} (Screen Net) & Horizon screen \(S^2\) with algebra net \(P \mapsto \mathcal{A}(P)\) & PAPER.md §1.3 \\
\textbf{A2} (Overlap Consistency) & Compatible marginals on overlaps & PAPER.md §1.3 \\
\textbf{A3} (Generalized Entropy) & Quantum focusing, \(S_{\mathrm{gen}}\) monotonicity & PAPER.md §1.3 \\
\textbf{A4} (MaxEnt) & State selected by constrained entropy maximization & PAPER.md §1.3 \\
\textbf{R0} (Finite-dim regulator) & Local Hilbert spaces are finite-dimensional & PAPER.md §2.5 \\
\textbf{R1} (Boundary gauge structure) & Region algebras are gauge-invariant subalgebras & PAPER.md §2.5 \\
\textbf{\([z]=0\)} & Loop-coherent gluing / DHR transportability & PAPER.md §6.6, Prop. 6.1a \\
...
\emph{Proof.} Admissibility condition (iii) requires light chiral matter. By Corollary 6.8 (PAPER.md §6.3), refinement stability forces the matter content to be chiral. To support chiral fermions that can form gauge-invariant Yukawa couplings with a single Higgs doublet (condition iv), the gauge group must admit both:
...
\textbf{From:} Lemmas 6.3--6.5 of PAPER.md, applied to the minimal carrier.
...
\emph{Proof.} This is Lemma 6.5 of PAPER.md. The hypercharge assignments must form a continuous family (not a discrete group) to allow the full range of Yukawa couplings needed for condition (iv). \(\square\)
...
\emph{Proof.} Theorem 6.13 of PAPER.md derives the unique anomaly-free hypercharge assignment for \(SU(3) \times SU(2) \times U(1)\) with \(N_c = 3\) and \(N_g = 3\), assuming single-Higgs Yukawa completeness (condition iv). The result is the standard sixth-integer lattice. \(\square\)
...
\tightlist
\item
  \textbf{Fermion mass hierarchy and Yukawa couplings} , see \url{TECHNICAL_SUPPLEMENT.md} §12
\item
  \textbf{Cosmological constant} , see \url{TECHNICAL_SUPPLEMENT.md} §14
\item
  \textbf{Full transmutation chain} , see \url{TECHNICAL_SUPPLEMENT.md} Appendix A.3
\item
  \textbf{Heat-kernel parameter \(t\) from UV microphysics} , see \url{PAPER.md} §6.13
\item
  \textbf{Particle mass spectrum from pixel geometry} , see \url{SPECTRUM_DERIVATION.md}
\end{itemize}
