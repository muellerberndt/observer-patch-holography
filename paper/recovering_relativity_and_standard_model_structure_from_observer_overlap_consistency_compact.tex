...
\begin{remark}
The five axioms fix the OPH core, and in the present packaging they already internalize the quasi-local propagation and refinement-stable notion of sector persistence used by the scaling-limit and gauge branches. The cap-modular geometric action and the \(2\pi\) KMS normalization are recovered in Theorem~\ref{thm:bw}; what remains genuinely theorem-external is the retained nine-item technical package: the null half-sided inclusion step \textbf{T6}, the obstruction clause \textbf{T7} when transportability is invoked, the scaling-limit scope clause \textbf{T8}, fixed-cap stationarity \textbf{T9}, and the bosonic/symmetric/fiber-functor part of \textbf{T10}--\textbf{T12}, together with \textbf{T1} and \textbf{T4} where the fixed-cutoff regulator presentation and tangent fixed-point comparison are used. In particular, the Lorentz/null-modular/Einstein and compact-gauge statements below are still scaling-limit claims rather than fixed-cutoff matrix-algebra identities.
\end{remark}
