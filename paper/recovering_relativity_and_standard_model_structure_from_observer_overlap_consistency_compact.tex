...
The gravity chain still requires a bridge from modular data to a local stress tensor, but the sharp claim boundary is now the following. At fixed regulator scale one can transfer the same edge-center data used in the spatial collar theorem to regulated null strips. What does not follow from that transfer alone is the left/right tensor split of the strip multiplicity spaces needed for exact splice and modular-additivity identities. The null bridge therefore separates three layers:
\begin{enumerate}[leftmargin=2em]
...
\item exact Markovity, or a controlled family converging to the exact Markov set on one fixed finite-dimensional strip model.
\end{enumerate}
Only after all three layers are in place does the null strip use the same HJPW factorization and modular-additivity argument as the spatial collar branch. The later identification of the resulting half-line density with a continuum null stress tensor remains a separate scaling-limit premise under Assumption~\ref{ass:technical}\textbf{T8}.
...
\begin{corollary}[Controlled null modular additivity on an inherited strip model]\label{cor:n1}
...
2\sqrt{\varepsilon}.
\]
Hence exact null modular additivity is available only at exact Markovity, or along a controlled strip family on one fixed inherited strip model for which
\[
...
\]
and the modular Hamiltonian is additive on each block. The only non-additive remainder is therefore the direct sum of block constants, which is central and gives \(K_{\partial,J}\).
...
Under the half-sided modular inclusion premise of Corollary~\ref{cor:n2}, there exists a one-parameter unitary group \(U(a)=e^{iaP}\) with \(P\ge 0\) such that
\[
...
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Assumption~\ref{ass:technical}, Proposition~\ref{prop:nullec}, the exact-or-controlled Markov strip hypothesis of Corollary~\ref{cor:n1}, Proposition~\ref{prop:n3}, the differentiability hypotheses of Theorem~\ref{lem:density}, the half-sided inclusion premise \textbf{T6}, and the scaling-limit null-stress identification premise of the relativistic EFT branch. Then there exists an operator-valued distribution \(T_{kk}(v,\Omega)\) such that the renormalized modular Hamiltonian on a null half-line takes the form
\[
\widetilde K[(a,\infty),\Omega]
=
